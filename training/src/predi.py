...
def predict(model,tokeniser,text,mod=1):
    inputs = tokeniser(
            text,
...
def convert (model,tokeniser,text,inv_vocab,mod=1):
    predictions,inputs= predict(model,tokeniser,text,mod=mod)
...
def reconstruction(model,tokeniser,fe,inv_vocab,mod=1):
    new_la= []
    for i in range(len(fe)):
        tokens,pred_labels,word_ids=convert (model,tokeniser,fe[i],inv_vocab,mod=mod)
        pred_la= reconstruction_per_sentence(pred_labels,fe[i],word_ids)
        new_la.append(pred_la)
    return new_la

def performance(model,tokeniser,data,inv_vocab,mod=1):
    true_label,pred_label=[],[]
    for i in range(len(data)):
        fe ,la,_ = decomposition_and_labelisation(data,i)
        la_pred= reconstruction(model,tokeniser,fe,inv_vocab,mod=mod)
        true_label+= la
        pred_label+=la_pred
...
if __name__ == "__main__":
    ...
    model.eval()
    precision_test ,recall_test,f1_test= performance(model,tokeniser,datas_test,inv_vocab,mod=1)
    precision_ev ,recall_ev,f1_ev= performance(model,tokeniser,datas_eval,inv_vocab,mod=1)
